  ...
  <body class="home page-template-default page page-id-13 wp-embed-responsive cooltimeline-body no-sidebar">
    ...
    <script>
        ...
        document.addEventListener('visibilitychange', () => {
            if (document.visibilityState === 'visible') {
                ...
                if (visibilityStartTime) {
                    totalVisibleTime += new Date() - visibilityStartTime;
                }
            }
        });
        ...
        window.addEventListener('beforeunload', () => {
            if (document.visibilityState === 'visible' && visibilityStartTime) {
                totalVisibleTime += new Date() - visibilityStartTime;
            }
            const endTime = new Date();
            const timeSpent = endTime - startTime; // 毫秒数
            const timeSpentSeconds = Math.round(timeSpent / 1000); // 转换为秒
            const totalVisibleTimeSeconds = Math.round(totalVisibleTime / 1000); // 转换为秒

            // 创建AJAX请求
            const xhr = new XMLHttpRequest();
            xhr.open("POST", "log_time.php", true);
            xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
            xhr.send(`timeSpent=${timeSpentSeconds}&visibleTime=${totalVisibleTimeSeconds}`);
        });
    </script>
            ...
            <div class="ctl-col-6">
              <div class="story-details">
                <div class="story-image">
                  <img src="./index_files/screenshot-0628_Interim_Defence.png">
                </div>
                <h3 style="color:#000;font-size:15px;line-height:20.1px">Design and Implementation of an Embodied Intelligent Agent for Nursing Robots Using Multimodal Large Models to Enhance Patient Care and Interaction. </h3>
                <p style="color:#000;font-size:12px;line-height:20.76px">
                  ...
                  <a href="./index_files/0628_Interim_Defence.pdf">Presentation</a><br>
                </p>
              </div>
            </div>
          </div>
